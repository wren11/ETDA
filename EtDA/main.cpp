...
DWORD WINAPI PacketRecvConsumer(LPVOID Args)
{
	while (true)
	{
		...
		try
		{
			if (*((int*)0x00721000) == 1)
			{
				int target = *((int*)0x00721004);
				int length = *((int*)0x00721008);
				unsigned char *data = (unsigned char*)0x00721012;
				...
				else
				{
					...
					*((int*)0x00721000) = 0;
					*((int*)0x00721012) = 0;
					WriteProcessMemory(GetCurrentProcess(), (void*)0x00721012, 0x00, 2048, NULL);
				}
			}
		}
		catch (exception)
